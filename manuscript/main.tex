...
A modern foundation-model application is no longer a single model queried in isolation: a deployed assistant or enterprise analyst tool stitches retrieval-augmented generation (RAG), dense and lexical indexes, rerankers, supervised fine-tuning and LoRA adapters, -data generation, and caches into one pipeline. RAG connects language models to external knowledge sources~\cite{lewis2020rag,karpukhin2020dpr,izacard2021fid}, while instruction tuning and adapter-based specialization fold private or domain-specific records directly into model behavior. This improves utility but creates a core trustworthy-AI problem: after a deletion request, an operator can remove the original record while the deployed system keeps behaving as if it remained---through retrieved chunks, embeddings, summaries, question--answer pairs, cache entries, adapters, or evaluation traces.

Privacy and governance regimes motivate data erasure and deletion accountability~\cite{voigt2017gdpr,fan2025fedfmchallenges}, and machine unlearning provides the algorithmic foundation for making systems forget selected data~\cite{cao2015machineunlearning,ginart2019making,bourtoule2021machineunlearning,guo2020certified,yao2024largeunlearning,liu2024rethinking,jin2024surveyunlearning}. But in modern LLM and RAG deployments the deletion problem is not confined to the parameter vector: a system can pass a model-centric unlearning test while still retrieving a stale chunk, citing a shadow copy, answering from a derivative, or paraphrasing a fact learned through an adapter. Extraction and membership-inference work shows that such dependence can be exposed through black-box interaction~\cite{carlini2019secretsharer,carlini2021extracting,carlini2023quantifying,shokri2017membership,yeom2018privacy,mattern2023membership}, and RAG evaluation studies retrieval quality and attribution~\cite{asai2024selfrag}; yet none directly answers the operator's audit question: \textit{after a deletion request, what measurable evidence shows that the deleted data and its downstream derivatives no longer influence retrieval, generation, or fine-tuned behavior?}
...
This article proposes \method{} (\emph{Auditable Data-Centric Unlearning}), a framework for deletion-risk auditing in RAG and fine-tuned foundation-model systems. \method{} represents deletion targets and their downstream artifacts in a provenance graph; defines an $(\varepsilon,\delta)$ behavioral-deletion guarantee as bounded distance between $S_-$ and $S_{\mathrm{clean}}$ over an adversary-aware probe class; allocates probes through a graph-structured sequential test; and uses direct, paraphrase, retrieval, counterfactual, extraction, trigger, and graph-pivot probes as diagnostic localizers rather than as the failure definition. Instead of a binary claim of perfect forgetting, it returns a confidence-bounded pass, fail, or escalate decision under an operator-chosen tolerance. The goal is not to replace unlearning algorithms, certified removal, differential privacy, or legal review~\cite{dwork2006dp,guo2020certified,kirchenbauer2023watermark}, but to provide measurable technical evidence about residual behavioral dependence in the pipeline surrounding those mechanisms.

We instantiate the framework in \bench{} across four core tracks: \emph{RealRAG} loads the FEVER corpus~\cite{thorne2018fever} as a public retain set, injects private targets, and contrasts provenance-guided purges with index-only and cache-forgetful deletions; \emph{NaturalFEVER}~\cite{thorne2018fever} treats public FEVER passages themselves as deletion units and audits whether raw evidence, citation caches, or answer-only paraphrases still influence responses; \emph{FineTuneSim} stresses adapter-memory residue under gradient-ascent, negative fine-tuning, over-unlearning, and replacement failures; and \emph{HybridReal} combines RAG retention with derivatives that survive through summaries, cache artifacts, graph-pivot expansion, or model-side instruction examples.
We further add LoRA-SFT~\cite{hu2021lora} and cached PEFT/LoRA validations, pretrained LoRA margin analysis, TOFU-style forget/retain evaluation~\cite{maini2024tofu}, neural dense retrieval (MiniLM/E5/BGE), clean-counterfactual two-sample tests, and component interventions for causal mediation. Across the repeated harness, \method{} detects configured deletion failures in RealRAG, NaturalFEVER, and HybridReal with zero configured false alarms; in FineTuneSim it detects 72.2\% of all configured failures, or 96.3\% of residual adapter-memory failures once utility-only over-unlearning cases are separated as escalations. The results do not prove legal compliance or absolute absence of influence; they show that deletion audits become substantially more informative when they are clean-counterfactual, provenance-aware, graph-adaptive, and multi-channel.

In summary, this paper (i)~formulates deletion verification for RAG and fine-tuned foundation-model systems as $(\varepsilon,\delta)$ counterfactual behavioral indistinguishability from a clean reference pipeline; (ii)~defines deletion risk as clean-counterfactual distance with the six evidence channels as diagnostic localizers rather than as circular ground truth; (iii)~presents a graph-structured adaptive audit algorithm with confidence bounds over provenance-linked target--channel arms, together with causal intervention diagnostics that decompose residual risk into retrieval-, cache-, derivative-, and parametric-mediated components; and (iv)~introduces \bench{}, a reusable benchmark with four tracks (\emph{RealRAG}~\cite{thorne2018fever}, \emph{NaturalFEVER}~\cite{thorne2018fever}, \emph{FineTuneSim}, \emph{HybridReal}) plus LoRA-SFT/PEFT~\cite{hu2021lora}, dense-retrieval, and TOFU-style~\cite{maini2024tofu} extensions, with an attack and baseline suite showing where exact-match, canary-only, retriever-only, membership-inference, extraction-only, and influence-proxy audits fail.
...
The counterfactual gold standard is a clean reference system $S_{\mathrm{clean}}(D_{\mathrm{del}})$ built as if the deletion targets had never entered any pipeline component: no raw record, chunk, embedding, cache item, derivative, adapter update, evaluation trace, or summary is produced from $D_{\mathrm{del}}$. Exact construction of $S_{\mathrm{clean}}$ may require retraining, rebuilding indexes, regenerating data, and purging caches. When that construction is infeasible, it remains the formal oracle that defines deletion success; practical audits estimate distance to this oracle.
...
\end{align}
The excess score is not a new guarantee; it is a calibrated way to state how far the observed risk rises above the clean-UCB floor. A floor-normalized operating tolerance $\tau_{\mathrm{ex}}$ corresponds to the raw-UCB condition $\mathrm{UCB}_{\mathrm{cf}}(z)\leq r_0+(1-r_0)\tau_{\mathrm{ex}}$. The experiments report both raw $\mathrm{UCB}_{\mathrm{cf}}$ and $\widetilde R_{\mathrm{cf}}$, and the headline pass/fail rule uses the declared excess-risk tolerance so clean systems with zero empirical counterfactual distance are not rejected only because the audit budget is finite. If $S_{\mathrm{clean}}$ is expensive, the audit can use an archived clean index, filtered retraining on a small slice, or a clean simulator for benchmark evaluation. The article reports both the counterfactual statistic when the clean reference is available and the channel scores used to diagnose why a system is distinguishable.
...
\]
The derivative-coupling term $\gamma_z$ (Fig.~\ref{fig:composition}) is zero exactly when deletion is provenance-closed: every chunk, embedding, summary, cache entry, example, and adapter-training derivative of the record has been removed or rendered behaviorally inert. Thus component-level unlearning guarantees compose only after lineage closure; otherwise orphan derivatives can dominate residual behavior. Appendix~A gives the full theorem, multi-stage extension, and necessity proof.

\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=4mm and 6mm, every node/.style={font=\scriptsize, align=center}]
 \node[adcunode, fill=adcuBlue!10, draw=adcuBlue, text width=20mm] (theta) {Parametric\\error $\varepsilon_\theta$};
 \node[adcunode, fill=adcuGreen!10, draw=adcuGreen, text width=20mm, right=of theta] (K) {Knowledge\\error $\varepsilon_K$};
 \node[adcunode, fill=adcuOrange!12, draw=adcuOrange, text width=20mm, right=of K] (gen) {Generation\\Lipschitz $L$};
 \node[adcudec, below=5mm of K, text width=58mm] (bound) {Composed bound:\\ $\rho_{\mathcal{B}}(S^-) \le \varepsilon_\theta + L\,\varepsilon_K + \gamma_z$};
 \node[adcunode, fill=adcuRed!15, draw=adcuRed, text width=22mm, right=7mm of bound] (gz) {derivative coupling $\gamma_z$\\(orphan derivatives, non-additive)};
 \draw[adcuflow] (theta)--(K);
 \draw[adcuflow] (K)--(gen);
 \draw[adcuflow] (theta) to[bend right=12] (bound);
 \draw[adcuflow] (K) to[bend right=6] (bound);
 \draw[adcuflow] (gen) to[bend left=12] (bound);
 \draw[adcuflow, dashed] (gz) -- (bound);
\end{tikzpicture}}
\caption{Pipeline-deletion composition (Appendix~A). Per-component deletion errors compose additively---$\varepsilon_\theta$ on parameters, $\varepsilon_K$ on knowledge amplified by the generation Lipschitz factor $L$---plus a non-additive derivative-coupling term $\gamma_z$ that is zero \emph{iff} every derivative of the deleted record has also been removed; otherwise orphan derivatives dominate.}
...
We evaluate \method{} in two stages. First, we use a compact pilot to validate the metric, audit algorithm, benchmark schema, and attack suite. Second, we run a repeated submission-scale harness with a real-corpus FEVER RAG retain set, natural FEVER evidence deletion cases, simulated fine-tuning memories, and a hybrid RAG-to-derivative setting. The repeated harness spans five random seeds, three deletion-target sizes, three audit budgets, and the reviewer-response black-box/counterfactual modes, producing 6,840 audit-result rows.

\textbf{Tracks.}
The four tracks introduced in Section~1 are stressed with concrete deletion modes: \emph{RealRAG}~\cite{thorne2018fever} contrasts provenance-guided, index-only, shadow-copy, cache-not-purged, and backdoor-triggered deletions; \emph{NaturalFEVER} treats public FEVER passages as deletion units and asks whether raw evidence, citation caches, summaries, or answer-only paraphrases still influence responses; \emph{FineTuneSim} models adapter-memory residue under filtered retraining, gradient-ascent unlearning, negative fine-tuning, replacement failure, and over-unlearning; and \emph{HybridReal} layers private derivatives onto FEVER-style RAG retention through examples, cache artifacts, graph-pivot expansion, and model-side behavior.
...
\method{} complements rather than replaces certified removal, deletion-as-control, differential privacy, or pan-private continual release: it audits the deployed behavior and derivative artifacts that sit outside the formal-guarantee boundary, treating any component-level guarantee as prior evidence and spending more probes on uncovered downstream artifacts.
...
This section collects the detailed empirical tables and figures moved out of the main paper to reduce density. They support the summarized claims in Section~7 while keeping the primary narrative focused on the counterfactual deletion definition, audit algorithm, calibration figures, and headline detection result. In particular, Fig.~\ref{fig:channels} visualizes the six diagnostic evidence channels referenced in Section~4; Fig.~\ref{fig:hybrid_detection} gives per-method HybridReal detection and Fig.~\ref{fig:budget} the audit-budget--detection tradeoff referenced in Section~7; Table~\ref{tab:adcu_residual} reports the per-deletion-method residual-dependence channels, and Table~\ref{tab:per_seed} the per-seed detection and risk used to separate average performance from seed-level instability.

\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[node distance=2mm and 3mm, every node/.style={font=\scriptsize, align=center}]
 \node[adcunode, fill=adcuRed!18, draw=adcuRed] (d) {$e_d$\\direct};
 \node[adcunode, fill=adcuRed!12, draw=adcuRed, right=of d] (p) {$e_p$\\paraphrase};
 \node[adcunode, fill=adcuOrange!18, draw=adcuOrange, right=of p] (r) {$e_r$\\retrieval};
 \node[adcunode, fill=adcuPurple!12, draw=adcuPurple, right=of r] (c) {$e_c$\\counter-\\factual};
 \node[adcunode, fill=adcuBlue!12, draw=adcuBlue, right=of c] (x) {$e_x$\\extraction};
 \node[adcunode, fill=adcuGray!18, draw=adcuGray, right=of x] (w) {$e_w$\\watermark};
 \node[adcuclean, below=5mm of c.south, text width=48mm] (risk) {evidence vector $e(S_-,q,z) \to$ channel score $\hat R$ (localizers; the failure definition is the $S_-$-vs-$S_{\mathrm{clean}}$ distance)};
 \foreach \n in {d,p,r,c,x,w} \draw[adcuflow, line width=0.4pt] (\n)--(risk);
\end{tikzpicture}}
\caption{The six diagnostic evidence channels referenced in Section~4. Each fires under a different failure mode---direct/paraphrase under model-side leakage, retrieval under stale or shadow-copy artifacts, counterfactual under any $S_-$-vs-$S_{\mathrm{clean}}$ divergence, extraction under adversarial probes, watermark under canary or provenance-token hits---and together they localize residual dependence without themselves defining deletion failure.}
\label{fig:channels}
\end{figure}
...
\begin{axis}[
 ...
 xmin=8, xmax=52, ymin=0.90, ymax=1.05, xtick={12,24,48},
 tick label style={font=\tiny}, label style={font=\scriptsize},
 grid=major, major grid style={gray!18}, mark size=1.6pt]
\addplot[fill=adcuBlue, draw=none, fill opacity=0.15, forget plot] coordinates {(12,1.0198)(24,1.0370)(48,1.0370)(48,0.9630)(24,0.9630)(12,0.9358)};
...
\addplot[fill=adcuOrange, draw=none, fill opacity=0.15, forget plot] coordinates {(12,0.8201)(24,0.7080)(48,0.6257)(48,0.5657)(24,0.6560)(12,0.7821)};
...
\textbf{Data release.}
The public retain data comes from local FEVER evidence files. The private deletion targets are generated from seeds. The benchmark should release the generation code and seeds rather than a fixed private-data dump. This makes it possible to regenerate targets, vary target counts, and avoid accidental normalization of sensitive-looking strings in public artifacts.

\textbf{Audit reports.}
Each audit report includes the decision, aggregate risk UCB, per-target UCBs, empirical risk, priorities, number of probes, violating probe identifiers, retain utility, and notes. For article tables, protected content is summarized through channel scores and redacted case-study rows.
