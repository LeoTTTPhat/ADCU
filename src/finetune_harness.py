from __future__ import annotations

from .benchmark import SyntheticADCUScenario, make_clean_system, make_leaky_system
from .metrics import ExperimentRow, evaluate_audit_method
from .rag_harness import AUDIT_METHODS


def run_finetune_track(scenario: SyntheticADCUScenario) -> list[ExperimentRow]:
    cases = [
        (
            "FineTune",
            "filtered adapter retraining",
            make_clean_system(scenario),
            False,
            0.99,
        ),
        (
            "FineTune",
            "gradient-ascent unlearning",
            make_leaky_system(
                scenario,
                leak_target_ids={"user_beta"},
                retrieve_target_ids=set(),
                paraphrase_only=True,
                canary_suppressed=True,
            ),
            True,
            0.97,
        ),
        (
            "FineTune",
            "adapter inventory only",
            make_leaky_system(scenario, leak_target_ids={"user_gamma"}, retrieve_target_ids=set()),
            True,
            0.97,
        ),
        (
            "FineTune",
            "over-unlearned adapter",
            make_clean_system(scenario),
            True,
            0.40,
        ),
    ]
    rows: list[ExperimentRow] = []
    for track, deletion_method, system, failed, retain_utility in cases:
        for audit_method in AUDIT_METHODS:
            rows.append(
                evaluate_audit_method(
                    scenario.graph,
                    system,
                    scenario.targets,
                    track,
                    deletion_method,
                    audit_method,
                    failed,
                    retain_utility,
                )
            )
    return rows

