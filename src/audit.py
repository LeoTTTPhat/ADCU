...
from .risk import RiskWeights, aggregate_risk, confidence_radius, mean_risk, risk_ucb, valuation_priorities
...
class AuditReport:
    ...
    aggregate_empirical_risk: float = 0.0
    aggregate_confidence_floor: float = 0.0
    floor_normalized_risk: float = 0.0
    delta: float = 0.05
...
class ADCUAuditor:
    ...
    def audit(
        ...
        per_target_floor = {
            target_id: confidence_radius(len(scores), self.delta)
            for target_id, scores in evidence.items()
        }
        aggregate = aggregate_risk(per_target_ucb, priorities)
        aggregate_empirical = aggregate_risk(per_target_empirical, priorities)
        aggregate_floor = aggregate_risk(per_target_floor, priorities)
        normalized = 0.0
        if aggregate_floor < 1.0:
            normalized = max(0.0, min(1.0, (aggregate - aggregate_floor) / (1.0 - aggregate_floor)))
        ...
        if violating_probes or normalized > self.global_tolerance:
            decision = "fail"
        elif retain_ok:
            decision = "pass"
        else:
        ...
        return AuditReport(
            ...
            aggregate_empirical_risk=aggregate_empirical,
            aggregate_confidence_floor=aggregate_floor,
            floor_normalized_risk=normalized,
            delta=self.delta,
