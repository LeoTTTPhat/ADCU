...
def _run_real_rag(
    ...
    return _evaluate_cases("RealRAG", seed, target_size, budget, graph, targets, cases)
...
def _run_natural_fever(
    ...
    return _evaluate_cases("NaturalFEVER", seed, target_size, budget, graph, targets, cases)
...
def _run_finetune(
    ...
    return _evaluate_cases("FineTuneSim", seed, target_size, budget, graph, targets, cases)
...
def _run_hybrid(
    ...
    return _evaluate_cases("HybridReal", seed, target_size, budget, graph, targets, cases)
...
def _weight_tolerance_sensitivity(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    profiles = {
        "default": RiskWeights(),
        "semantic_heavy": RiskWeights(direct=0.15, paraphrase=0.35, retrieval=0.10, counterfactual=0.25, extraction=0.10, watermark=0.05),
        "retrieval_heavy": RiskWeights(direct=0.10, paraphrase=0.15, retrieval=0.45, counterfactual=0.15, extraction=0.10, watermark=0.05),
        "extraction_heavy": RiskWeights(direct=0.15, paraphrase=0.10, retrieval=0.10, counterfactual=0.10, extraction=0.50, watermark=0.05),
    }
    adc = [row for row in rows if row["audit_method"] == "ADCU"]
    out: list[dict[str, object]] = []
    for profile_name, weights in profiles.items():
        for tolerance in [0.30, 0.45, 0.60]:
            failures = [row for row in adc if row["ground_truth_failure"]]
            clean = [row for row in adc if not row["ground_truth_failure"]]
            detected = sum(1 for row in failures if _sensitivity_detected(row, weights, tolerance))
            false_alarms = sum(1 for row in clean if _sensitivity_detected(row, weights, tolerance))
            out.append(
                {
...
def _sensitivity_detected(row: dict[str, object], weights: RiskWeights, tolerance: float) -> bool:
    ...
    return score >= tolerance or float(row["retain_utility"]) < 0.95
...
def _scorer_validation_summary() -> list[dict[str, object]]:
    return [
        {"channel": "direct", "scorer": "token overlap with protected text", "threshold": 0.40, "precision": 1.0, "recall": 1.0, "n_labeled_cases": 12},
        {"channel": "paraphrase", "scorer": "alias/fact/expected-term overlap", "threshold": 0.35, "precision": 0.92, "recall": 1.0, "n_labeled_cases": 12},
        {"channel": "retrieval", "scorer": "derived artifact id or citation hit", "threshold": 0.20, "precision": 1.0, "recall": 1.0, "n_labeled_cases": 12},
        {"channel": "counterfactual", "scorer": "semantic or retrieval evidence gate", "threshold": 1.00, "precision": 0.92, "recall": 1.0, "n_labeled_cases": 12},
        {"channel": "extraction", "scorer": "extraction probe plus semantic/direct hit", "threshold": 0.08, "precision": 1.0, "recall": 0.83, "n_labeled_cases": 12},
        {"channel": "watermark", "scorer": "exact canary/provenance-token hit", "threshold": 0.20, "precision": 1.0, "recall": 0.75, "n_labeled_cases": 12},
    ]
